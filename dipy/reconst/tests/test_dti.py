...
def test_tensor_scalar_attributes():
    """
    Test whether the tensor class scalar attributes (FA, ADC, etc...) are
    calculated properly.
    ...
    assert_almost_equal(np.abs(np.dot(evecs[:, 2],
                        ...
                        err_msg="Third eigenvector is not right")
    assert_array_almost_equal(D, tensor[0].D,
                              err_msg="Recovery of self diffusion tensor from "
                                      "eig not adaquate")
    assert_almost_equal(FA, tensor.fa(),
                        err_msg="Calculation of FA of self diffusion tensor "
                                "is not adequate")
    assert_almost_equal(MD, tensor.md(),
                        err_msg="Calculation of MD of self diffusion tensor "
                                "is not adequate")
    assert_(tensor.mask.all())
...
def test_fa_of_zero():
    ...
    with warnings.catch_warnings():
        ...
        assert_(np.isnan(ten.fa(nonans=False)))
...
def test_WLS_and_LS_fit():
    ...
    # Recall: D = [Dxx,Dyy,Dzz,Dxy,Dxz,Dyz] and D ~ 10^-4 mm^2 /s
    b0 = 1000.
    ...
    assert_array_almost_equal(tensor_est.lower_triangular, D)
...
def test_masked_array_with_Tensor():
    ...
    assert_(isinstance(tensor.model_params, MaskedView))
...
def test_init():
    ...
    gtab, bval = read_bvec_file(get_data('55dir_grad.bvec'))
    dti.Tensor(data, bval, gtab.T, mask, thresh=0)
    mask[:] = False
