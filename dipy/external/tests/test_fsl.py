...
def test_flirt_behavior():
    ...
    assert_array_almost_equal(res0, np.eye(4), decimal=2)
    ...
    assert_array_almost_equal(res1, flipx, decimal=2)
    # Either way round
    res2 = fsl.flirt(pos_det, ONE_VOL)
    assert_array_almost_equal(res2, flipx, decimal=2)
    # pos det to self obviously also identity
    res3 = fsl.flirt(pos_det, pos_det)
    assert_array_almost_equal(res3, np.eye(4), decimal=2)
